...
class GeneralSim(HasTraits):
	...
	statedata=Instance(ISim) #Stack as stored in reflectance portion of code
...
class LayerVfrac(MaterialSim): 
	...
	def runsim(self): 
		...
		for i in range(self.inc):
			...
			self.statedata.update_simview()         #Recompute Reflectance



	#		svl_dic[str(vf)]=self.statedata.simview.get_sexy_data()
