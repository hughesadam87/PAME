...
class FiberView(HasTraits):
    ...
    traits_view = View(Item('FiberScene',
                        editor=SceneEditor(scene_class=MayaviScene), style='custom', show_label=True),		                      		
	...
	outter_tube=self.FiberScene.mlab.points3d(0, 0, 0, color=(.84, .84, .84), opacity=.5, mode='cylinder',
		extent=[-1, 1,-.1, .1, -.1, .1] )  #Extent controls xmax,xmin,ymax etc.. all relative.  Must be -a to a for centered 
	inner_tube=self.FiberScene.mlab.points3d(0, 0, 0, color=(.77, .77, .77), mode='cylinder',
