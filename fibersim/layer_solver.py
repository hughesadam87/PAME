...
def to_rads(angle_in_degrees): return angle_in_degrees*(math.pi/180.0)

def to_degrees(angle_in_rad):  return angle_in_rad* (180.0 / math.pi)

def snelly(angle_1, n_arrays, lambdas):
	'''Takes in arrays of index (assuming order from 0 up is left to right); computes angles (possibly complex) via Snells'''
...
def fressy(ang_array, n_arrays, TM_or_TE, lambdas):
	...
	jmax=fressy.shape[0]
	TM_list=['tm', 'TM', 'Tm']; TE_list=['te', 'TE', 'Te']; Mixed=['mixed', 'Mixed',]; valid_list=TM_list+TE_list+Mixed
