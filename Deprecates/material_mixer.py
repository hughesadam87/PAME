...
class DoubleMixer(HasTraits):
	...
	specparms=Instance(SpecParms,())        #maybe not necessary, but ensures everything is hooked up.

	lambdas=DelegatesTo('specparms')   #So defaults are declared automatically
	...
	def _esolvent_changed(self): self.update_mix()
	def _esolute_changed(self): self.update_mix()
	def _Vfrac_changed(self): self.update_mix()
		
	def _mixedarray_default(self):	return np.empty((self.lambdas.shape[0]), dtype='complex')
...
class MG_Mod(DoubleMixer):
	...
	traits_view=View( 
			...
			HGroup( Item('Vfrac'), Item('K')), #Item('mixedarray')
			)
	...
	def update_mix(self):
		...
		for i in range(len(self.esolvent)):
			...
			B=self.Vfrac*epi
			beta=(1.0/3.0)          #1/3 for spherical particles, not sure for others!!!
			gam=(1.0/(3.0*emr) ) + (self.K/(4.0*math.pi*emr))
			C=em + beta*(epr - emr) - self.Vfrac*gam*(epr-emr)
			D=beta*epi - self.Vfrac*gam*epi
...
class CustomEquiv(EquivMethod):
	'''Modification of the equivalence method to allow for a psuedo effective radius parameter, instead of just rcore + rshell'''
	mix_name='Custom Equiv Method'

	#Correction parameters for fine tuning shell, core to meet#
	alpha=Range(low=0.0,high=2.0,value=1.0) 
	beta=Range(low=0.0,hight=2.0,value=1.0)

	rcore_eff=Property(Float, depends_on=['alpha, r_particle'])
	rshell_eff=Property(Float, depends_on=['beta, r_shell'])

	traits_view=View(         #FOR SOME REASON 'shell_core_ratio' causes the view to crash!
			HGroup(Item('alpha'), Item('beta')), Item('r_particle'), Item('r_shell'), HGroup( Item('rcore_eff'), Item('rshell_eff')),# Item('shell_core_ratio') ),
			)

	def _rcore_eff_changed(self): self.update_mix()
	def _rshell_eff_changed(self): self.update_mix()

	def _get_rcore_eff(self): return self.alpha * self.r_particle
	def _get_rshell_eff(self): return self.beta * self.r_shell
