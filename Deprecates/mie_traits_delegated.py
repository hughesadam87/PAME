...
from traits.api import HasTraits, Any, Instance, Array, Property, CArray, Str, Float, Int, Button, Bool, Interface, implements, DelegatesTo, on_trait_change
...
#	Qext=Property(Array, depends_on='Cext')
	...
	traits_view=View(Include('basic_group'))
		...
		self.update_cross();self.update_sview()

#	def _CoreMaterial_changed(self): self.update_cross()
#	def _MediumMaterial_changed(self): self.update_cross()

	def _ecore_changed(self): 
		print 'detecting earray change in MIE'
		self.update_cross()

	def _emedium_changed(self): 
		print 'detecting earray change in MIE'
		self.update_cross()

	def _bessmax_changed(self): self.update_cross()
	def _cutoff_criteria_changed(self): self.update_cross()

	def update_cross(self): pass
	...
	def _sviewbutton_fired(self): 
		if self.ui is not None:         #Kills window if it's already open and update is fired
			self.ui.dispose() 
		self.sview.update(self.lambdas, self.Cext, self.Cscatt, self.Cabs, self.x_unit)	
		self.ui=self.sview.edit_traits()

	def _Cext_default(self): return empty(self.lambdas.shape[0], dtype='float')   
	def _Cabs_default(self): return empty(self.lambdas.shape[0], dtype='float')   
	def _Cscatt_default(self): return empty(self.lambdas.shape[0], dtype='float')   

	def update_cross(self): pass  #Function which computes cross section for each Mie object

	def _show_scatter_fired(self): self.update_cross()
	...
	def wave_vectors(self, narray): 
		'''This returns a set of wavevectors in and arbitrary medium defined by n.  Recall the wavevector is a function of medium as well as wavelength'''		
		return (2.0*math.pi*narray)/(self.lambdas)
...
class shell(Mie):
	...
	def _r_shell_changed(self): self.update_cross(); self.update_sview()
	def _ShellMaterial_changed(self): 
		self.update_cross()

	def _eshell_changed(self): 
		print 'IN MIE ESHELL CHANGED'
		self.update_cross()
...
class sphere_full(sphere):
	...
	traits_view=View(Include('sphere_full_group'), 
	...
	def update_cross(self):
		print 'in mie', self.CoreMaterial, self.MediumMaterial
		for i in range(len(self.lambdas)):   #Can't get rid of this because bessel functions can't generate with full arrays
...
if __name__ == '__main__':
	from material_traits_v3 import DrudeNew, Dispwater, Constant, Sellmeir  #For testing purposes
	from main_parms import SpecParms
	x=linspace(400,700,100)
	sp=SpecParms()

	e=DrudeNew(specparms=sp); f=Dispwater(specparms=sp) ; g=Sellmeir(specparms=sp)
	m=sphere_shell(specparms=sp, CoreMaterial=e, MediumMaterial=f , ShellMaterial=g)
#	m=sphere_full(specparms=sp, CoreMaterial=e, MediumMaterial=g)
	m.configure_traits()
